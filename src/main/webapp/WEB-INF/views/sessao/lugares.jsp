							...
							<table>
								...
								<c:forEach var="lugar" items="${map.value}">
									<td class="fileira-assento"><figure>
										<svg class="assento ${session.isAvailable(lugar) && !shopCart.isAvailable(lugar) ? 'disponivel' : 'ocupado'}" onclick="${session.isAvailable(lugar) && !shopCart.isAvailable(lugar) ? 'changeCheckbox(this)' : ''}" data-lugar="${lugar}" id="${seat.id}"  version="1.0" id="SEAT" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
